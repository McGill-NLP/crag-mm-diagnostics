...
class ModularEvaluation:
    ...
    def _load_system_prompt(self) -> str:
        ...
        if self.task_type == "whole":
            ...
            system_prompt = (
                "You are an expert image analyst specializing in visual question answering. Your task is to identify the precise region in an image that contains the visual information needed to answer a given question.\n\n"
                "CRITICAL INSTRUCTIONS:\n"
                "1. Analyze the question to understand what visual element or object it refers to.\n"
                "2. Locate the smallest rectangular region (bounding box) that contains the relevant visual information.\n"
                "3. Provide coordinates as [x1, y1, x2, y2] where:\n"
                "   - x1, y1 are the top-left coordinates\n"
                "   - x2, y2 are the bottom-right coordinates\n"
                "   - Coordinates are in pixel units (0 to image width/height)\n"
                "   - The bounding box should be as tight as possible around the target\n"
                "4. If the question refers to text/signs, include the entire readable area\n"
                "5. If the question refers to an object, include the full object with minimal background\n"
                "6. Output ONLY the coordinate list [x1, y1, x2, y2] - no additional text, explanations, or formatting\n"
                "7. Remember: Focus on the minimal region that contains all visual information needed to answer the question. Be precise with coordinates.\n\n"
                "EXAMPLES:\n"
                "Example 1:\n"
                "Question: 'What is the name of the store in the image?'\n"
                "Image: A street scene with a store on the left\n"
                "Output: [25, 25, 75, 75]\n\n"
                "Example 2:\n"
                "Question: 'What color is the car?'\n"
                "Image: A parking lot with a blue sedan in the center\n"
                "Output: [150, 200, 400, 350]\n\n"
                "Example 3:\n"
                "Question: 'How many windows does the building have?'\n"
                "Image: A building facade with multiple windows\n"
                "Output: [50, 100, 450, 300]"
            )
